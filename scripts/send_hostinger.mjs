...
async function sendEmailHostinger(email, firstName, lastName) {
  ...
  try {
    await transporter.sendMail({
      ...
      headers: {
        'List-Unsubscribe': '<https://plantayraiz.com.br/unsubscribe>',
      }
    });
    return true;
  } catch (err) {
...
async function main() {
  ...
  console.log('Buscando todos os contatos do Brevo (Lista 2)...');
  ...
  const toSend = missing; 
  console.log(`\n🎯 Encontramos ${missing.length} contatos que faltam. Iniciando envio via Hostinger (1 email a cada 60 seg)...`);
  
  const logFile = path.join(__dirname, '..', `campaign_log_hostinger_${Date.now()}.json`);
  const logData = [];
  ...
  for (let i = 0; i < toSend.length; i++) {
    ...
    console.log(`[${i+1}/${toSend.length}] Enviando para ${email}...`);
    const ok = await sendEmailHostinger(email, firstName, lastName);
    
    if (ok) {
      console.log(`  ✅ [Enviado via Hostinger] ${email}`);
      sentCount++;
      logData.push({ email, status: 'sent', date: new Date().toISOString() });
  ...
  console.log(`\n🎉 Disparo via Hostinger concluído! Foram enviados ${sentCount} emails.`);
}
