-- ============================================================
-- CATÁLOGO OFICIAL COMPLETO: PLANTA Y RAIZ LTDA (10 MEDICAMENTOS)
-- Execute este script no SQL Editor do Supabase para atualizar
-- todos os 10 medicamentos reais com 3 fotos e valores de mercado
-- ============================================================
...
DECLARE
  v_rec RECORD;
BEGIN
  -- 1. Atualizar todas as farmácias existentes para o nome, logo e fachada oficiais
  ...
  -- 2. Para cada farmácia ativa, limpar os produtos antigos e inserir os 10 medicamentos oficiais
  FOR v_rec IN SELECT id FROM public.vendors WHERE is_active = true LOOP
    
    -- Limpar produtos antigos deste vendor
    DELETE FROM public.vendor_products WHERE vendor_id = v_rec.id;

    -- Inserir os 10 medicamentos completos
...
Posologia Resumida: Para apetite: 2,5 mg antes do almoço e do jantar (5 mg/dia). Para náuseas pós-quimio: 5 mg/m² administrados 1 a 3 horas antes da sessão de quimioterapia, repetindo a cada 2 a 4 horas pós-tratamento.',
      1850.00,
      2100.00,
      'capsula',
