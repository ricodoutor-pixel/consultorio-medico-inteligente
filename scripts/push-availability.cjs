...
async function run() {
  console.log('🚀 Deploy Doctor Availability Manager\n');

  const msg = 'feat(doctor): Availability manager for weekly grid and dashboard alert';
  await pushFile('src/components/doctor/DoctorAvailabilityManager.tsx', msg);
  await pushFile('src/pages/ConfiguracoesMedico.tsx', msg);
