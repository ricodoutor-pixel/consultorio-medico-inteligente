...
async function run() {
  const files = [
    'src/lib/pharmacy-kyc-docs.ts',
    'src/components/admin/PharmacyKycDocViewer.tsx',
  ...
  for (const f of files) {
    await pushFile(f, 'feat: add KYC Lojas e Farmácias module in admin, registration, and quick access');
  }
}
