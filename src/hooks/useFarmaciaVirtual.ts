...
export function useFarmaciaVirtual() {
  ...
  const dispensePrescription = async (prescriptionId: string, trackingCode?: string) => {
    ...
    if (trackingCode) {
      updatePayload.tracking_code = trackingCode.trim();
    }
    const { error: updateErr } = await (supabase
      .from('prescriptions') as any)
    ...
    toast({
      ...
      description: trackingCode ? `Status atualizado com código de rastreamento: ${trackingCode}` : "O status foi registrado e o paciente notificado."
    });
    await loadVendorData();
  };
