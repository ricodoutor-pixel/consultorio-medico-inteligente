...
// Pacing: 30s | Dual-JID para Números Brasileiros | Timeout de 45s por HTTP Request
...
async function sendWahaText(chatId, text) {
  ...
  const timeoutId = setTimeout(() => controller.abort(), 45000); // 45s timeout para Railway WAHA
