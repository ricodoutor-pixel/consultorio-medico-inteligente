  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Perguntas Rápidas Regulatórias & Clínicas (após onboarding ou no chat livre) */}
        {onboardingStep >= 4 && (
          <div className="px-3 py-1.5 border-t border-border/50 bg-muted/20 flex gap-1.5 overflow-x-auto no-scrollbar">
            {[
              ...
              <button
                ...
                className="whitespace-nowrap px-2.5 py-1 rounded-full text-[11px] bg-background border border-primary/20 hover:border-primary text-foreground hover:bg-primary/10 transition-colors shrink-0 font-medium"
              >
                {q}
              </button>
            ))}
        ...
        {/* Input */}
        <div className="p-2.5 sm:p-3 border-t border-border flex flex-col gap-1.5 bg-card">
          <div className="flex gap-2">
            <input
              ...
              placeholder={onboardingStep < 4 ? "Responda a pergunta..." : "Digite sua dúvida sobre a Anvisa, saúde ou laudos..."}
              disabled={isStreaming}
              className="flex-1 bg-muted border border-border rounded-xl px-3 py-2 text-xs sm:text-sm text-foreground focus:outline-none focus:border-primary/50 transition-colors disabled:opacity-50"
            />
            <Button
              size="icon"
