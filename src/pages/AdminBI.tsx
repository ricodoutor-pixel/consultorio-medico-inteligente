...
const AdminBI = () => {
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    const [patientsRes, doctorsRes, appointmentsRes, triagesRes] = await Promise.all([
      supabase.from("profiles").select("id", { count: "exact", head: true }).eq("user_type", "patient"),
      supabase.from("doctors").select("id", { count: "exact", head: true }),
      supabase.from("appointments").select("amount, scheduled_at, status"),
    ...
    const triageConversion = totalTriages > 0 ? (completedTriages / totalTriages) * 100 : 0;

    // Simulated metrics (would come from marketing data in production)
    const cac = totalPatients > 0 ? Math.max(15, 500 / Math.max(totalPatients, 1)) : 0;
    const ltv = avgTicket * 4.2; // Average 4.2 consultations per patient
    const churnRate = 4.8;

    setMetrics({
      totalRevenue, totalPatients, totalDoctors, totalConsultations,
      avgTicket, triageConversion, cac, ltv, churnRate,
    });
    ...
    setConversionData([
      ...
      { name: "Consulta", value: totalConsultations },
      { name: "Recorrência", value: Math.floor(totalConsultations * 0.35) },
    ]);
