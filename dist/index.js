...
var saasPlans = mysqlTable("saas_plans", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 100 }).notNull(),
  slug: varchar("slug", { length: 50 }).notNull().unique(),
  monthlyPrice: decimal("monthlyPrice", { precision: 10, scale: 2 }).notNull(),
  description: text("description"),
  benefits: json("benefits").$type().notNull(),
...
var brisaTriages = mysqlTable("brisa_triages", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  symptoms: text("symptoms"),
  medicalHistory: text("medicalHistory"),
  location: varchar("location", { length: 255 }),
  matchedDoctors: json("matchedDoctors").$type(),
  recommendedProducts: json("recommendedProducts").$type(),
  triageDate: timestamp("triageDate").defaultNow().notNull(),
  followUpDate: timestamp("followUpDate"),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var smartRefills = mysqlTable("smart_refills", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
var anvisaValidations = mysqlTable("anvisa_validations", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
var supportTickets = mysqlTable("support_tickets", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
async function getSaasPlans() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(saasPlans).orderBy(saasPlans.monthlyPrice);
}
async function getSaasPlanBySlug(slug) {
  const db = await getDb();
  if (!db) return void 0;
