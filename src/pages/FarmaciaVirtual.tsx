...
export default function FarmaciaVirtual() {
  ...
  const [isDispensingLoading, setIsDispensingLoading] = useState(false);
      ...
      <main className="flex-1 container mx-auto px-4 pt-24 pb-16">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="receitas" className="space-y-6">
            ...
            {/* Modal de Confirmação de Dispensação com Rastreamento */}
            <Dialog open={!!dispensingRxId} onOpenChange={(open) => { if (!open) { setDispensingRxId(null); setTrackingCode(""); } }}>
              <DialogContent className="max-w-md bg-card border-border rounded-2xl">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2 text-lg font-bold">
                    <Truck className="text-emerald-400" /> Dispensar Receita & Envio
                  </DialogTitle>
                  <DialogDescription className="text-xs text-muted-foreground">
                    Informe o código de rastreamento da transportadora ou Correios para que o paciente acompanhe a entrega em tempo real.
                  </DialogDescription>
                </DialogHeader>

                <div className="py-3 space-y-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="trackingCodeInput" className="text-xs font-bold text-foreground">
                      Código de Rastreamento (Obrigatório) *
                    </Label>
                    <Input
                      id="trackingCodeInput"
                      placeholder="Ex: PYR-TRK-894102 ou BR123456789BR"
                      value={trackingCode}
                      onChange={(e) => setTrackingCode(e.target.value.toUpperCase())}
                      className="font-mono text-sm uppercase rounded-xl border-border bg-background"
                      autoFocus
                    />
                    <p className="text-[11px] text-muted-foreground">
                      Mínimo de 8 caracteres. {trackingCode.length > 0 && trackingCode.length < 8 && (
                        <span className="text-amber-400 font-bold ml-1">({trackingCode.length}/8 caracteres)</span>
                      )}
                    </p>
                  </div>
                </div>
                ...
                <DialogFooter className="gap-2">
                  <Button
                    ...
                    disabled={isDispensingLoading}
                  >
                    Cancelar
                  </Button>
                  <Button
                    size="sm"
                    className="bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-950/20"
                    disabled={trackingCode.trim().length < 8 || isDispensingLoading}
                    onClick={async () => {
                      if (!dispensingRxId || trackingCode.trim().length < 8) {
                        toast({ title: "Código de rastreamento inválido", description: "O código de rastreio deve ter no mínimo 8 caracteres.", variant: "destructive" });
                        return;
                      }
                      try {
                        setIsDispensingLoading(true);
                        await dispensePrescription(dispensingRxId, trackingCode.trim());
                        setDispensingRxId(null);
                        setTrackingCode("");
                      } catch (err: any) {
                        toast({ title: "Erro ao dispensar", description: err?.message || "Ocorreu um erro", variant: "destructive" });
                      } finally {
                        setIsDispensingLoading(false);
                      }
                    }}
                  >
                    {isDispensingLoading ? "Dispensando..." : "Confirmar Dispensação ✅"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
