export const testimonials = [
  {
    name: "Dona Maria",
    age: "62 anos",
    topic: "Artrite • Dor Crônica",
    quote: "Depois de 8 anos sofrendo com artrite reumatoide, o óleo de CBD mudou minha vida. Reduzi 70% dos analgésicos e voltei a fazer crochê. A consulta pela plataforma foi acessível e o médico explicou tudo com paciência.",
    imageUrl: "https://images.unsplash.com/photo-1566616213894-2d4e1baee5d8?w=200&h=200&fit=crop&crop=face",
  },
  {
    name: "Roberto",
    age: "45 anos",
    topic: "Epilepsia • Convulsões",
    quote: "Meu filho de 12 anos tinha até 15 crises por semana. Com o tratamento prescrito pelo neurologista da plataforma, as crises reduziram para 2 por mês. O CBD full spectrum foi um divisor de águas para nossa família.",
    imageUrl: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=200&h=200&fit=crop&crop=face",
  },
  {
    name: "Juliana",
    age: "34 anos",
    topic: "Ansiedade • Insônia",
    quote: "Tomava 3 remédios para dormir e acordava pior. A psiquiatra da plataforma ajustou meu protocolo com CBD e em 3 meses consegui dormir naturalmente. Hoje uso apenas o óleo e faço acompanhamento mensal online.",
    imageUrl: "https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=200&h=200&fit=crop&crop=face",
  },
  {
    name: "Sr. Antônio",
    age: "71 anos",
    topic: "Parkinson • Tremores",
    quote: "Os tremores melhoraram muito com o tratamento. Consigo comer sozinho de novo e minha esposa diz que voltei a sorrir. A telemedicina facilitou porque não preciso sair de casa. Preço justo e atendimento humano.",
    imageUrl: "https://images.unsplash.com/photo-1552058544-f2b08422138a?w=200&h=200&fit=crop&crop=face",
  },
  {
    name: "Carla",
    age: "28 anos",
    topic: "Fibromialgia • Fadiga",
    quote: "Vivi 5 anos com dores que ninguém entendia. Depois da consulta online com o especialista em dor, iniciei o protocolo com CBD + THC microdose. Em 2 meses voltei a trabalhar e fazer exercícios. Gratidão eterna!",
    imageUrl: "https://images.unsplash.com/photo-1589571894960-20bbe2828d0a?w=200&h=200&fit=crop&crop=face",
  },
];
