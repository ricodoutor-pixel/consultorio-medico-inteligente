  ...
  const handleTestAiHunter = async () => {
    ...
    try {
      ...
      if (!error && data?.ok) {
        ...
        toast.error("Falha ao processar lead: " + (data?.error || error?.message || "Não foi possível extrair dados"));
      }
    } catch (err: any) {
      toast.error("Erro na extração de lead: " + (err?.message || "Falha na conexão"));
    } finally {
      setTestingAi(false);
    }
  };
