...
async function run() {
  const files = [
    'src/lib/gemini-models-registry.ts',
    'scripts/auto-agent-optimizer.cjs',
    'src/pages/admin/AdminKycAgentes.tsx',
    'src/pages/Admin.tsx',
    'src/App.tsx',
  ];

  for (const f of files) {
    await pushFile(f, 'feat: add KYC Agentes e IAs 04h Auto-Optimizer mirroring Google AI Studio');
  }
}
