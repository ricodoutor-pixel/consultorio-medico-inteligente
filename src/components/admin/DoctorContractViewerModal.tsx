...
export default function DoctorContractViewerModal({ open, onClose, contract }: Props) {
  ...
  const crmFormatted = contract.doctor_crm ? `CRM-${contract.doctor_crm_uf || "SP"} ${contract.doctor_crm}` : "CRM não informado";
  const cpfFormatted = contract.doctor_cpf ? contract.doctor_cpf : "CPF não informado (⚠️ Pendente)";
  ...
  const signerIp = contract.signer_ip || "IP não capturado (Auditoria Pendente)";
  const sha512Hash = contract.sha512_hash || "Hash não disponível (Pendente de assinatura)";
