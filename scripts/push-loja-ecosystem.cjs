...
async function run() {
  console.log('🚀 Deploy Pharmacy Ecosystem Engine\n');

  const msg = 'feat(pharmacy): B2B2C marketplace engine, dashboard, kyc, and showcase';
  await pushFile('supabase/migrations/20260830000001_pharmacy_marketplace_engine.sql', msg);
  ...
  await pushFile('src/components/consultation/PrescriptionViewModal.tsx', msg);
  await pushFile('src/components/home/ProductShowcase.tsx', msg);
  await pushFile('src/components/ShoppingCart.tsx', msg);
