  ...
  const fetchOutcomes = useCallback(async () => {
    ...
    try {
      ...
      if (outcomes && outcomes.length > 0) {
        const chartData = (outcomes as unknown as Outcome[]).map((o) => ({
          dia: new Date(o.created_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }),
          nivel: Number(o.symptom_level) || 0,
          humor: o.mood || "neutro",
        }));
        setData(chartData);
    ...
    return (
      <Card className="border-border">
        <CardContent className={`${compact ? "p-4" : "p-6"} space-y-3`}>
          ...
          <Skeleton className={`w-full ${compact ? "h-[150px]" : "h-[220px]"}`} />
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="w-full overflow-hidden">
          <ResponsiveContainer width="100%" height={compact ? 150 : 220}>
            <LineChart data={data}>
              ...
              <XAxis dataKey="dia" stroke="hsl(240 10% 68%)" fontSize={10} tick={{ fontSize: 9 }} interval="preserveStartEnd" />
              <YAxis stroke="hsl(240 10% 68%)" fontSize={10} domain={[0, 10]} width={30} />
